...
def _arxiv_record_to_document(record: ArxivRecord) -> Document:
    """Map a fetched arXiv record into the shared Document model."""
...
def ingest_arxiv_records(
    store: SQLiteStore,
    records: list[ArxivRecord],
    qdrant_indexer: ChunkIndexer | None = None,
    neo4j_writer: GraphWriter | None = None,
) -> dict[str, int | str]:
    """Persist arXiv metadata and continue through the same downstream pipeline."""

    documents = [_arxiv_record_to_document(record) for record in records]
    written = store.upsert_documents(documents)
